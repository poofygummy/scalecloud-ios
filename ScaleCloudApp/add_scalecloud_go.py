#!/usr/bin/env python3
"""
Add ScaleCloudGo.xcframework file reference and link it to all targets.
...
def add_framework_to_project(pbxproj_path):
    ...
    # Check if ScaleCloudGo.xcframework already exists
    if 'ScaleCloudGo.xcframework' in content:
        print("ScaleCloudGo.xcframework reference already exists in project")
        return content
    
    # Generate UUIDs for the file reference and build file
    file_ref_uuid = generate_uuid()
    
    # Add file reference for ScaleCloudGo.xcframework
    ...
    file_ref_entry = f'\t\t{file_ref_uuid} /* ScaleCloudGo.xcframework */ = {{isa = PBXFileReference; lastKnownFileType = wrapper.xcframework; name = ScaleCloudGo.xcframework; path = ../ScaleCloudGo/prebuilt/ScaleCloudGo.xcframework; sourceTree = SOURCE_ROOT; }};\n'
    ...
    print(f"✓ Added PBXFileReference for ScaleCloudGo.xcframework ({file_ref_uuid})")
    
    # Now add ScaleCloudGo.xcframework to each PBXFrameworksBuildPhase
    ...
    for match in frameworks_phases:
        ...
        build_file_entry = f'\t\t{build_file_uuid} /* ScaleCloudGo.xcframework in Frameworks */ = {{isa = PBXBuildFile; fileRef = {file_ref_uuid} /* ScaleCloudGo.xcframework */; }};\n'
        ...
        if phase_match:
            ...
            new_files = old_files.rstrip() + f'\n\t\t\t\t{build_file_uuid} /* ScaleCloudGo.xcframework in Frameworks */,'
            content = content.replace(f'files = ({old_files})', f'files = ({new_files})', 1)
            added_count += 1
    
    print(f"✓ Added ScaleCloudGo.xcframework to {added_count} PBXFrameworksBuildPhase sections")
...
if __name__ == '__main__':
    ...
    print("Adding ScaleCloudGo.xcframework to ScaleCloudApp.xcodeproj...")
    ...
    print("\n✓ Done! ScaleCloudGo.xcframework has been added to all targets.")
