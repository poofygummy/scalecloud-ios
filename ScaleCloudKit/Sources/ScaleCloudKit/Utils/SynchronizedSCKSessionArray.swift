...
/// A thread-safe container for managing an array of `SCKSession` instances.
///
...
public final class SynchronizedSCKSessionArray: @unchecked Sendable {

    // MARK: - Internal Storage

    /// Internal storage for the session array.
    private var array: [SCKSession]
    ...
    /// - Parameter initial: An initial array of `SCKSession` to populate the container with.
    public init(_ initial: [SCKSession] = []) {
        self.array = initial
        self.queue = DispatchQueue(label: "com.nextcloud.SynchronizedSCKSessionArray", attributes: .concurrent)
    }
    ...
    public var all: [SCKSession] {
        queue.sync { array }
    }

    /// Calls the given closure on each session in the array, in order.
    /// - Parameter body: A closure that takes a `SCKSession` as a parameter.
    public func forEach(_ body: (SCKSession) -> Void) {
        queue.sync {
            array.forEach(body)
        }
    }
    ...
    /// - Returns: A `SCKSession` instance if found, otherwise `nil`.
    public func session(forAccount account: String) -> SCKSession? {
        queue.sync {
    ...
    ///   - newSession: The `SCKSession` instance that will replace the existing one.
    public func replace(account: String, with newSession: SCKSession) {
    ...
    /// - Parameter element: The `SCKSession` to append.
    public func append(_ element: SCKSession) {
        queue.async(flags: .barrier) {
    ...
    /// - Returns: A `SCKSession` if the index is valid, otherwise `nil`.
    public subscript(index: Int) -> SCKSession? {
        queue.sync {
