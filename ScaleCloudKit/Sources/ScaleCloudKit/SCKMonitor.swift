...
//  Description:
//
//  SCKMonitor is an Alamofire EventMonitor implementation used to observe
...
//
//  SCKMonitor is intended as an observational and diagnostic component and does not
//  modify request execution or response handling.
//

final class SCKMonitor: EventMonitor {
    internal let nkCommonInstance: SCKCommon
    internal let queue = DispatchQueue(label: "com.nextcloud.SCKMonitor", qos: .utility)
    ...
    func requestDidResume(_ request: Request) {
        ...
        queue.async {
            switch SCKLogFileManager.shared.logLevel {
            case .normal:
