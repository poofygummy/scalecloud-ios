...
/// SCKOperationHandle is an actor, so interactions are serialized and safe across concurrency domains.
///
/// Features:
...
/// - Observe lifecycle events with `events()` returning `AsyncStream<SCKOperationEvent>`.
...
public actor SCKOperationHandle {
    private(set) var request: DataRequest?
    private(set) var task: URLSessionTask?

    public enum SCKOperationEvent {
    ...
    private var eventsStream: AsyncStream<SCKOperationEvent>?
    private var eventsContinuation: AsyncStream<SCKOperationEvent>.Continuation?

    public init() {}

    public func events() -> AsyncStream<SCKOperationEvent> {
        if let eventsStream { return eventsStream }
        let (stream, continuation) = AsyncStream<SCKOperationEvent>.makeStream()
