...
///
public struct SCKLock: Equatable, Sendable {
    ///
    ...
    ///
    public let ownerType: SCKLockType
    ...
    public init?(xml properties: XML.Accessor) {
        ...
        guard let rawOwnerTypeValue = properties["nc:lock-owner-type"].int, let lockOwnerType = SCKLockType(rawValue: rawOwnerTypeValue) else {
            return nil
        }
    ...
    ///
    public init(owner: String, ownerEditor: String, ownerType: SCKLockType, ownerDisplayName: String, time: Date?, timeOut: Date?, token: String?) {
        self.owner = owner
