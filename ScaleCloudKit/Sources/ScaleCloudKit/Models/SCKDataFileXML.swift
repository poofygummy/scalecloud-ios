...
public class SCKDataFileXML: NSObject {
    var nkCommonInstance: SCKCommon
    ...
    public func getRequestBodyFile(createProperties: [SCKProperties]?, removeProperties: [SCKProperties] = []) -> String {
        let request = """
        <?xml version=\"1.0\" encoding=\"UTF-8\"?>
        <d:propfind xmlns:d=\"DAV:\" xmlns:oc=\"http://owncloud.org/ns\" xmlns:nc=\"http://nextcloud.org/ns\">
            <d:prop>
        """ + SCKProperties.properties(createProperties: createProperties, removeProperties: removeProperties) + """
            </d:prop>
        </d:propfind>
        """
        return request
    ...
    func getRequestBodyFileListingFavorites(createProperties: [SCKProperties]?, removeProperties: [SCKProperties] = []) -> String {
        let request = """
        <?xml version=\"1.0\"?>
    ...
    func getRequestBodySearchFileName(createProperties: [SCKProperties]?, removeProperties: [SCKProperties] = []) -> String {
        let request = """
        <?xml version=\"1.0\"?>
    ...
    func getRequestBodySearchFileId(createProperties: [SCKProperties]?, removeProperties: [SCKProperties] = []) -> String {
        let request = """
        <?xml version=\"1.0\"?>
        <d:searchrequest xmlns:d=\"DAV:\" xmlns:oc=\"http://owncloud.org/ns\" xmlns:nc=\"http://nextcloud.org/ns\">
        <d:basicsearch>
    ...
    func getRequestBodySearchMedia(createProperties: [SCKProperties]?, removeProperties: [SCKProperties] = []) -> String {
        let request = """
        <?xml version=\"1.0\"?>
        <d:searchrequest xmlns:d=\"DAV:\" xmlns:oc=\"http://owncloud.org/ns\" xmlns:nc=\"http://nextcloud.org/ns\">
        <d:basicsearch>
    ...
    func convertDataFile(xmlData: Data, nkSession: SCKSession, rootFileName: String, showHiddenFiles: Bool, includeHiddenFiles: [String]) async -> [SCKFile] {
        var files: [SCKFile] = []
        ...
        for element in elements {
            ...
            let tags: [SCKTag] = SCKTag.parse(systemTagElements: propstat["d:prop", "nc:system-tags", "nc:system-tag"])
            ...
            for downloadLimit in propstat["d:prop", "nc:share-download-limits", "nc:share-download-limit"] {
                ...
                file.downloadLimits.append(SCKDownloadLimit(count: count, limit: limit, token: token))
            }
        ...
        for index in files.indices {
            ...
            if index < files.count - 1,
               ...
               files[index].classFile == SCKTypeClassFile.image.rawValue,
               files[index + 1].classFile == SCKTypeClassFile.video.rawValue {
    ...
    func convertDataTrash(xmlData: Data, nkSession: SCKSession, showHiddenFiles: Bool) async -> [SCKTrash] {
        var files: [SCKTrash] = []
        var first: Bool = true
        guard let baseUrl = self.nkCommonInstance.getHostName(urlString: nkSession.urlBase) else {
            return files
    ...
    func convertDataComments(xmlData: Data) -> [SCKComments] {
        var items: [SCKComments] = []
        let xml = XML.parse(xmlData)
        let elements = xml["d:multistatus", "d:response"]

        for element in elements {
            let item = SCKComments()
