...
public class SCKTextProcessingTask {
    public var id: Int?
    ...
    static func deserialize(multipleObjects data: JSON) -> [SCKTextProcessingTask]? {
        guard let allResults = data.array else { return nil }
        return allResults.compactMap(SCKTextProcessingTask.init)
    }

    static func deserialize(singleObject data: JSON) -> SCKTextProcessingTask? {
        SCKTextProcessingTask(json: data)
    }

    public static func toV2(tasks: [SCKTextProcessingTask]) -> TaskList {
