...
public class SCKTextProcessingTaskType {
    ...
    static func deserialize(multipleObjects data: JSON) -> [SCKTextProcessingTaskType]? {
        guard let allResults = data.array else { return nil }
        return allResults.compactMap(SCKTextProcessingTaskType.init)
    }

    public static func toV2(type: [SCKTextProcessingTaskType]) -> TaskTypes {
